...
const LegacyVerificationResults = ({ results, onApprove, onReject, onFlag }) => {
  const [selectedTab, setSelectedTab] = useState('overview');

  // Safety check - if no results, return early
  if (!results) {
    return (
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'no_db': return 'text-muted-foreground';
  ...
  const getStatusBg = (status) => {
    switch (status) {
      ...
      case 'no_db': return 'bg-muted border-border';
  ...
  console.log('LegacyVerificationResults - results:', results);
  console.log('LegacyVerificationResults - extractedFields:', extractedFields);
            ...
            <div>
              <h3 className="text-xl font-semibold text-foreground capitalize">
                {results?.status === 'verified' ? 'Valid Certificate' : 
                 ...
                 results?.status === 'error' ? 'Verification Error' :
                 'No Database Record'}
              </h3>
              <p className="text-muted-foreground">
                {results?.message || 'Legacy database verification completed'}
              </p>
            </div>
          </div>
            ...
            <div className="p-4 bg-muted/30 border border-border rounded-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium text-foreground">Legacy CSV Database</p>
                </div>
                <div>
                  <span className="text-muted-foreground">Path:</span>
                  <p className="font-medium text-foreground truncate">
                    {results?.data?.path || results?.path || 'N/A'}
                  </p>
                </div>
                <div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-border">
                      ...
                      <th className="text-left py-2 text-sm font-medium text-muted-foreground">Database</th>
                        ...
                        <td className="py-3">
                          <div className="flex items-center space-x-2">
                            ...
                            <span className={`text-sm font-medium capitalize ${getFieldStatusColor(data?.match)}`}>
                              {data?.match === true ? 'Match' : 
                               data?.match === false ? 'Mismatch' : 
                               data?.db ? 'No Match' : 'Null'}
                            </span>
                          </div>
                        </td>
                      </tr>
...
export default LegacyVerificationResults;
